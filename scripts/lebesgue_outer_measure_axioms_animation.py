...
CANVAS = (760, 1490)
PANEL_CANVAS = (720, 480)
PANEL_BOX = (20, 20, 700, 460)
...
FONT_FORMULA = load_font(22, bold=True)
FONT_BODY = load_font(20)
FONT_SMALL = load_font(18)
FONT_LABEL = load_font(31, bold=True)
...
def draw_number_line(draw: ImageDraw.ImageDraw, panel: tuple[int, int, int, int], y: int, max_value: float) -> None:
    ...
    centered_text(draw, (x0 - 21, y), "0", FONT_AXIS, COLORS["muted"])
    centered_text(draw, (x1 + 24, y), "∞", FONT_AXIS, COLORS["muted"])
...
def arrow_to_measure(
    ...
    *,
    ...
    centered_text(draw, (end[0] + label_dx, end[1] - 34), label, FONT_SMALL, color)
...
def draw_nonnegative(draw: ImageDraw.ImageDraw, panel: tuple[int, int, int, int], frame: int) -> None:
    ...
    for i, (cx, cy) in enumerate(centers):
        ...
        if active:
            ...
            centered_text(draw, (px, py), label, FONT_LABEL, COLORS["ink"])
...
def draw_subadditive(draw: ImageDraw.ImageDraw, panel: tuple[int, int, int, int], frame: int) -> None:
    ...
    for i, (cx, cy) in enumerate(centers):
        ...
        if active:
            centered_text(draw, (px, py), f"A{subscript_int(i + 1)}", FONT_LABEL, "#ffffff")
    union_est = [0.11, 0.18, 0.24, 0.29, 0.33, 0.36][frame]
    sum_est = sum(values[:active_count])
    centered_text(draw, (panel[0] + 182, panel[1] + 315), "合併の外測度", FONT_SMALL, COLORS["purple"])
    centered_text(draw, (panel[0] + 365, panel[1] + 315), "各外測度の和", FONT_SMALL, COLORS["red"])
    arrow_to_measure(
        draw,
        (panel[0] + 182, panel[1] + 345),
        measure_x(panel, y_line, union_est, 0.95),
        COLORS["purple"],
        f"{MEASURE_SYMBOL}(A₁∪...)",
        label_dx=-24,
    )
    arrow_to_measure(
        draw,
        (panel[0] + 365, panel[1] + 345),
        measure_x(panel, y_line, sum_est, 0.95),
        COLORS["red"],
        f"{MEASURE_SYMBOL}(A₁)+...",
        label_dx=38,
    )
...
def draw_frame(frame: int) -> Image.Image:
    ...
    panels = [(40, 92, 720, 532), (40, 557, 720, 997), (40, 1022, 720, 1462)]
...
def draw_single_panel(draw_func, frame: int) -> Image.Image:
    image = Image.new("RGBA", PANEL_CANVAS, COLORS["paper"])
    draw_func(ImageDraw.Draw(image), PANEL_BOX, frame)
    return image
...
def build_animation() -> None:
    ...
    panel_specs = [
        ("nonnegative", draw_nonnegative),
        ("monotone", draw_monotone),
        ("subadditive", draw_subadditive),
    ]
    panel_frames: dict[str, list[Image.Image]] = {name: [] for name, _ in panel_specs}
    for frame_index in range(FRAME_COUNT):
        frame = draw_frame(frame_index)
        frame.save(FRAMES_DIR / f"frame_{frame_index:03d}.png")
        frames.append(frame.convert("P", palette=Image.ADAPTIVE))
        for name, draw_func in panel_specs:
            panel_frame = draw_single_panel(draw_func, frame_index)
            panel_frame.save(FRAMES_DIR / f"{name}_{frame_index:03d}.png")
            panel_frames[name].append(panel_frame.convert("P", palette=Image.ADAPTIVE))
    frames[0].save(
        GIF_PATH,
        save_all=True,
    ...
    for name, single_frames in panel_frames.items():
        single_path = GIF_DIR / f"{GIF_PATH.stem}_{name}.gif"
        single_frames[0].save(
            single_path,
            save_all=True,
            append_images=single_frames[1:],
            duration=FRAME_DURATION_MS,
            loop=LOOP,
            disposal=2,
            optimize=False,
        )
        print(f"Saved GIF to: {single_path.resolve()}")
