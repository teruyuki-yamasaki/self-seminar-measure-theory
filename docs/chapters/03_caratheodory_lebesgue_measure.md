...
すなわち, $\mathfrak{M}_\Gamma$ は可算加法族であり, 次が成り立つ.

1. $\emptyset\in\mathfrak{M}_\Gamma$.
2. $E\in\mathfrak{M}_\Gamma$ ならば $E^c\in\mathfrak{M}_\Gamma$.
3. $E_1, E_2, \ldots\in\mathfrak{M}_\Gamma$ ならば
...
$\mu^*$ を $\mathfrak{M}_{\mu^*}$ に制限したものを **Lebesgue 測度** と呼び,

$$
\mu := \mu^*|_{\mathfrak{M}_{\mu^*}}:\mathfrak{M}_{\mu^*}\to[0, \infty]
$$

と書く. Carathéodory の定理により, $\mathfrak{M}_{\mu^*}$ は可算加法族であり, $\mu$ はその上の可算加法的な測度である.
