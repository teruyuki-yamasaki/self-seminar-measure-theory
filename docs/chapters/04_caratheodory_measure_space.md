...
$A_1\subset A_2\subset\cdots$ とする. このとき,
...
また, $A_1\supset A_2\supset\cdots$ かつ $\mu(A_1)<\infty$ とする. このとき,
...
有限性の仮定 $\mu(A_1)<\infty$ は重要である. 減少列では差集合を使って下からの連続性へ帰着するため, 無限大から無限大を引く状況を避ける必要がある.

これらは集合列の極限と測度が整合する基本性質である.
...
| 空間 | 可測集合族 | 測度 | 意味 |
| --- | --- | --- | --- |
| 抽象的な測度空間 $X$ | 可算加法族 $\mathfrak{B}$ | $\mu$ | 集合 $A\in\mathfrak{B}$ に大きさ $\mu(A)$ を与える |
| ユークリッド空間 $\mathbb{R}^N$ | Lebesgue 可測集合族 $\mathfrak{M}_{\mu^*}$ | Lebesgue 測度 $\mu$ | 図形や点集合に長さ・面積・体積を与える |
| 確率空間 $\Omega$ | 事象の可算加法族 $\mathfrak{B}$ | 確率測度 $P$ | 事象 $A\in\mathfrak{B}$ に確率 $P(A)$ を与え, $P(\Omega)=1$ とする |
