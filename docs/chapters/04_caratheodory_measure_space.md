...
| 場面 | 空間 | 可測集合族 | 測度 | 意味 |
| --- | --- | --- | --- | --- |
| 抽象的な測度空間 | $X$ | $\mathfrak{B}$ | $\mu$ | 集合 $A\in\mathfrak{B}$ に大きさ $\mu(A)$ を与える |
| ユークリッド空間と Lebesgue 測度 | $\mathbb{R}^N$ | $\mathfrak{M}_{\mu^*}$ | Lebesgue 測度 $\mu$ | 図形や点集合に長さ・面積・体積を与える |
| 全体空間と確率 | $\Omega$ | 事象の集合族 $\mathfrak{B}$ | 確率測度 $P$ | 事象 $A\in\mathfrak{B}$ に確率 $P(A)$ を与え, $P(\Omega)=1$ とする |
