...
すなわち, $f,g\in V_1$ と $a,b\in\mathbb{R}$ に対して $af+bg \in V_1$ である. したがって, $V_1$ は実数体上のベクトル空間をなす.
...
このように, $V_1$ の上では a.e. で一致する二つの函数の距離が $0$ になる. したがって, 真のノルム空間を得るためには, そのような函数を同一視するのが自然である.
...
という関係を入れる. これは同値関係であるから, 各 $f\in V_1$ に対してその同値類 $[f]$ を考えることができる. さらに, a.e. で等しい函数は積分値を変えないので, 和と実数倍
...
は同値類の取り方によらずよく定まる. よって, 商空間 $V_1/{\sim}$ も実数体上のベクトル空間になる.
...
\|f-g\|_p=0,
\qquad
L^p(\mu):=V_p/{\sim_p}=\{[f]_p\mid f\in V_p\}
$$
...
と定めると, これは代表元の取り方によらず, $L^p(\mu)$ 上のノルムになる.
