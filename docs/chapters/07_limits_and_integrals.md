...
前章で見たように, Riemann 積分可能な函数は Lebesgue 積分可能であり, 積分値も一致する. したがって, ここで問題にするのは積分値の定義を変えたことではなく, 極限操作に対する安定性である.
...
Riemann 積分でも, 各点収束だけでは極限と積分の交換は保証されない. 古典的な十分条件は一様収束である.
...
であり, 一様収束により右辺は $0$ に近づく.

一様収束は, すべての点で誤差を同時に小さくする強い条件である. Lebesgue 積分の収束定理は, この強い条件をそのまま要求するのではなく, 測度論的な条件に置き換える. 単調収束定理は非負性と単調性で交換を保証し, Fatou の補題は非負性のもとで不等式を残し, 優収束定理は可積分な支配函数によって交換を保証する.

先ほどのスパイク列 $f_n=n\mathbf{1}_{(0, 1/n)}$ は一様収束していない. さらに, すべての $f_n$ を上から押さえる可積分函数も存在しない. そのため, Riemann 積分の一様収束条件からも, Lebesgue 積分の優収束定理からも外れる例になっている.
