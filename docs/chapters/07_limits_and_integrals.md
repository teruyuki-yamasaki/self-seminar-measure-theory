...
この例では, 台 $(0,1/n]$ は縮んでいくが, 高さ $n$ が大きくなるため, 面積は常に $1$ のままである. 点ごとの収束だけでは, 積分量が狭い領域に集中する現象を制御できない.
...
各点収束とは異なり, 同じ $N$ が $X$ のすべての点に対して効く. つまり, 函数列全体の誤差を一様に制御する強い収束概念である.
...
これは, 非負函数列が下から増加して極限函数に近づくとき, 積分値も下から増加して極限の積分に近づくという定理である.
...
単調収束定理では単調な函数列について等式が得られた. Fatou の補題では, 一般の非負函数列に対して, 極限函数の積分を評価できる.
...
をすべての $n$ で満たすなら, 可積分函数による支配のもとでは, 極限と積分を交換できる.
