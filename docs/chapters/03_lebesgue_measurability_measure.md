...
この考えを区間 $I$ だけでなく, 任意の集合 $B\subset\mathbb{R}^N$ に対して要求することで, Lebesgue 可測性が定義される.

![外測度と内外近似](../../figures/measure/animations/lebesgue_inner_outer_overlap/gif/lebesgue_inner_outer_overlap.gif)

## Lebesgue 可測集合

集合 $E\subset\mathbb{R}^N$ が, 任意の集合 $B\subset\mathbb{R}^N$ に対して
...
\mathfrak{M}_{\mu^*}
:=
\{E\subset\mathbb{R}^N \mid
\forall B\subset\mathbb{R}^N,\quad
...
外測度の可算劣加法性から, 任意の $B,E\subset\mathbb{R}^N$ について
...
1. $\emptyset\in\mathfrak{M}_{\mu^*}$.
2. $E\in\mathfrak{M}_{\mu^*}$ ならば $E^c\in\mathfrak{M}_{\mu^*}$.
3. $E_1,E_2,\ldots\in\mathfrak{M}_{\mu^*}$ ならば

$$
\bigcup_{n=1}^{\infty}E_n\in\mathfrak{M}_{\mu^*}.
$$

さらに, Lebesgue 外測度 $\mu^*$ を Lebesgue 可測集合族 $\mathfrak{M}_{\mu^*}$ に制限したもの
