...
本発表の流れは, まず集合に"大きさ"を与える理論を整え, そのうえで函数を積分し, 最後に極限操作との整合性を扱う.
...
導入では, この流れがなぜ必要になるのかを Riemann 積分の限界から考える.
...
ただし, 以下は積分の考え方を示すための直観的な説明である.

### Riemann 積分の考え方

**Riemann 積分**では, **定義域** $[a,b] \subset \mathbb{R}$ を分割
...
に, その区間での函数値の代表値 $f(\xi_i) \ (\xi_i \in \Delta_i)$ をかけて Riemann 和
...
各値域区間 $\Theta_k$ に対して, その**逆像** ( $y_k \le f(x) < y_{k+1}$ なる点 $x \in [a,b]$ の集合 )
...
ここで, $m_i \le M_i$ が常に成り立つので,
$$
S_*(f,\Delta) \le S^*(f,\Delta)
$$
...
このあとで問題になるのは, たとえば函数列 $f_n$ が函数 $f$ に収束する状況である.
...
Riemann 積分では, たとえば $f_n$ が $f$ に一様収束し, 各 $f_n$ が Riemann 積分可能ならば, 極限函数 $f$ も Riemann 積分可能であり積分と極限を交換できる. しかし一様収束は強い条件であり, 各点収束や単調増加極限のような自然な極限操作を十分に扱えない.

Lebesgue 積分論では, 単調収束定理や優収束定理により, 一様収束より広い状況でこの交換が可能になる. つまり, 個々の函数だけでなく, **函数列の極限を安定に扱う枠組み**が必要になる.
...
各 $G_n$ は有限集合であるから, $g_n$ が値 1 を取る点は離散的な有限個の点だけである.
...
一方, Lebesgue 積分ではこのような単調増加極限を単調収束定理で扱える.
