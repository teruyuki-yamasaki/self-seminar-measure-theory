...
$\mu^*(N)=0$ ならば $N$ は Lebesgue 可測である.

また, 可算集合は Lebesgue 外測度 $0$ であるから可測である.
...
- Lebesgue 外測度は任意集合に定義されるが, そのままでは測度ではない.
- Carathéodory 型の条件を満たす集合を可測集合として取り出す.
- 可測集合上に外測度を制限することで, 可算加法的な Lebesgue 測度が得られる.
